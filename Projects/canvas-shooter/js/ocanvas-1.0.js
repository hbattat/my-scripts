...
var enemy = function (settings, thecore) {
		...
		diesnow: function(){
			...
			//var die = setTimeout(function(){
				...
				enemies.died(this.key);
			//},200);
			
			
		},
		interval: 0,
		// Function for logic that chases player
		chase : function(){
			...
			// Collision Detection
			if(valueInRange(x, player.x, player.x + player.radius)){
				if(valueInRange(y, player.y, player.y + player.radius)){
					player.takeHit(enemy.damage);
				 	return false;
				}
			}
			
			//console.log('enemy.chase.interval');
			
			// var slopes = slope(x,player.x,y, player.y);
			
			
			var hyp = get_hyp(x,player.x,y, player.y);
	...
	// Register the display object
	oCanvas.registerDisplayObject("enemy", enemy);
